...
> 成熟形态的分布式 AI Agent 任务执行平台架构文档。
>
> 产品实施路线以 [MVP Pyramid](./stage/README.md) 为准。新的 Stage 0 是 [最小聊天 MVP](./stage/STAGE0_SIMPLE_CHAT.md)，不是本文描述的完整分布式执行地基。本文主要对应 Stage 5-8 的服务化、沙箱、多节点和生产运营能力。
...
SiriusX 现在按 MVP pyramid 推进：每个 stage 都是完整可用产品，只是后一层能力更强。详细说明见 [stage/README.md](./stage/README.md)。

| Stage | 产品形态 | 与本文关系 |
|---:|---|---|
| 0 | 最小聊天 MVP | 不需要本文的大部分分布式架构 |
| 1 | 本机工具助手 MVP | 只使用本机工具，不需要沙箱运行时 |
| 2 | 本地工作区 MVP | 引入 workspace 和 artifact，但仍是单机产品 |
| 3 | 可恢复任务 MVP | 引入 Task / Run / Event 模型 |
| 4 | 个人/小团队产品 MVP | 引入 Auth、历史和 Agent Catalog |
| 5 | 服务化执行 MVP | 开始使用 API / Worker、共享存储、队列 |
| 6 | 安全执行 MVP | 引入 sandbox runtime 和能力令牌 |
| 7 | 多节点 SaaS MVP | 引入 run lease、多 Worker 恢复、多租户 |
| 8 | 生产运营 MVP | 引入监控、告警、备份、审计 |

因此，本文不是起步阶段规格，而是后期目标架构参考。
...
- **安全边界优先**：Auth 和租户隔离是多用户/分布式阶段的基础，所有用户态资源从 authContext 派生权限
...
- **Stateless Sandbox**（后期分布式执行阶段）：每个 run 创建全新 sandbox，用完即销毁，无节点亲和性
...
**🚨 P1 修正：删除 sandbox_leases 表（后期分布式执行阶段 Stateless）**
...
Sandbox-runtime repo: siriusx-sandbox-runtime
  ...
  |-- LocalDockerSandboxProvider      后期分布式执行阶段实现
  |-- gVisor / MicroVM providers      未来扩展
  `-- event and commit proposal stream
```

**🚨 后期分布式执行阶段不需要 sandbox_leases 表**
...
| **Auth session** | **Redis** | **多 API 节点共享（🆕 多用户阶段）** |
...
**Sandbox 状态（后期分布式执行阶段 Stateless）**：
- 无 `sandbox_leases` 表
